...
function isTerminalApprovalInteractionStatus(status: string) {
...
function assertApprovalInteractionTargetsCandidate(interaction: IssueThreadInteraction, candidate: CandidateRow) {
...
export function releaseCandidateService(db: Db) {
  ...
  return {
    ...
    markApprovalInteractionCreated: async (
      ...
      if (candidate.approvalInteractionId || candidate.status !== "candidate_created") {
        const previousInteraction = await db
          .select()
          .from(issueThreadInteractions)
          .where(eq(issueThreadInteractions.id, candidate.approvalInteractionId ?? "00000000-0000-4000-8000-000000000000"))
          .then((rows) => rows[0] ?? null);
        const previous = previousInteraction
          ? ({
              id: previousInteraction.id,
              kind: previousInteraction.kind,
              status: previousInteraction.status,
              payload: previousInteraction.payload,
    ...
    buildApprovalInteractionInput: (candidate: CandidateRow, options: { reissueFromInteractionId?: string | null } = {}) => ({
      kind: "request_confirmation" as const,
      idempotencyKey: [
        `release-candidate:${candidate.id}:approval:${candidate.imageDigest}`,
        options.reissueFromInteractionId ? `reissue:${options.reissueFromInteractionId}` : null,
      ].filter(Boolean).join(":"),
    ...
    getApprovalReissueSource: async (candidate: CandidateRow) => {
      ...
      const row = await db
        .select()
        .from(issueThreadInteractions)
        .where(eq(issueThreadInteractions.id, candidate.approvalInteractionId))
        .then((rows) => rows[0] ?? null);
      if (!row) {
        throw conflict("Release candidate approval interaction is missing", {
          code: "release_candidate_approval_missing",
          candidateId: candidate.id,
          approvalInteractionId: candidate.approvalInteractionId,
        });
      }
      const interaction = {
        id: row.id,

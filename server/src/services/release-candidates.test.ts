...
describe("release candidate approval relay", () => {
  ...
  it("builds a fresh idempotency key for candidate approval reissues", () => {
    const { db } = makeDb();
    const input = releaseCandidateService(db).buildApprovalInteractionInput(candidate, {
      reissueFromInteractionId: "ffffffff-ffff-4fff-8fff-ffffffffffff",
    });

    expect(input.idempotencyKey).toBe(
      `release-candidate:${candidate.id}:approval:${candidate.imageDigest}:reissue:ffffffff-ffff-4fff-8fff-ffffffffffff`,
    );
    expect(input.payload.target).toMatchObject({
      type: "custom",
      key: `release_candidate:${candidate.id}`,
  ...
  it("rebinds approval to a new candidate-native interaction after the previous one expired", async () => {
    ...
    const previousInteraction = {
      ...
      result: { version: 1 as const, outcome: "superseded_by_comment" as const },
      createdAt: new Date(),
      updatedAt: new Date(),
    };
    const nextInteraction = {
      ...
      status: "pending" as const,
      idempotencyKey: `release-candidate:${candidate.id}:approval:${candidate.imageDigest}:reissue:${previousInteractionId}`,
      result: null,
    };
    const { db, inserted, updated } = makeDb([[approvalRequestedCandidate], [previousInteraction]]);

    const result = await releaseCandidateService(db).markApprovalInteractionCreated(
      candidate.id,
      nextInteraction,
      { agentId: candidate.createdByAgentId, runId: candidate.createdByRunId },
    );
    ...
    expect(updated).toEqual(expect.arrayContaining([
      expect.objectContaining({ approvalInteractionId: nextInteractionId }),
    ]));
  ...
  it("rejects approval reissue while the current candidate-native interaction is still pending", async () => {
    ...
    await expect(
      releaseCandidateService(makeDb([[previousInteraction]]).db)
        .getApprovalReissueSource(approvalRequestedCandidate),
    ).rejects.toMatchObject({
      status: 409,
      message: "Release candidate approval interaction is still pending",
    });
  });

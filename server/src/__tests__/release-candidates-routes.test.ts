...
const mockReleaseCandidateService = vi.hoisted(() => ({
  getById: vi.fn(),
  buildApprovalInteractionInput: vi.fn(),
  getApprovalReissueSource: vi.fn(),
  markApprovalInteractionCreated: vi.fn(),
...
describe("release candidate routes", () => {
  beforeEach(() => {
    ...
    mockReleaseCandidateService.buildApprovalInteractionInput.mockReset();
    mockReleaseCandidateService.getApprovalReissueSource.mockReset();
    mockReleaseCandidateService.markApprovalInteractionCreated.mockReset();
  ...
  it("reissues candidate-native approval interactions with a fresh idempotency key after supersession", async () => {
    ...
    const newInteractionId = "11111111-1111-4111-8111-111111111111";
    const candidate = {
      id: candidateId,
      companyId,
      sourceIssueId,
    ...
    const interactionInput = {
      ...
      idempotencyKey: `release-candidate:${candidateId}:approval:${candidate.imageDigest}:reissue:${previousInteractionId}`,
      payload: {
        version: 1 as const,
        prompt: "Approve?",
        target: {
    ...
    mockReleaseCandidateService.getById.mockResolvedValue(candidate);
    mockReleaseCandidateService.getApprovalReissueSource.mockResolvedValue(previousInteraction);
    mockReleaseCandidateService.buildApprovalInteractionInput.mockReturnValue(interactionInput);
    mockIssueService.getById.mockResolvedValue({ id: sourceIssueId, companyId });
    mockIssueThreadInteractionService.create.mockResolvedValue(createdInteraction);
    mockReleaseCandidateService.markApprovalInteractionCreated.mockResolvedValue({
      ...candidate,
      approvalInteractionId: newInteractionId,
    });
    const db = { transaction: async (callback: (tx: unknown) => Promise<unknown>) => callback({}) };
    const app = express();
    ...
    app.use("/api", releaseCandidateRoutes(db as never, {} as never));
    app.use(errorHandler);

    const res = await request(app)
      .post(`/api/release-candidates/${candidateId}/approval-interaction`)
      .send({});

    expect(res.status).toBe(201);
    expect(mockReleaseCandidateService.getApprovalReissueSource).toHaveBeenCalledWith(candidate);
    expect(mockReleaseCandidateService.buildApprovalInteractionInput).toHaveBeenCalledWith(candidate, {
      reissueFromInteractionId: previousInteractionId,
    });
    expect(mockIssueThreadInteractionService.create).toHaveBeenCalledWith(
      { id: sourceIssueId, companyId },
      expect.objectContaining({ idempotencyKey: interactionInput.idempotencyKey }),
      expect.any(Object),
    );
    expect(mockReleaseCandidateService.markApprovalInteractionCreated).toHaveBeenCalledWith(
      candidateId,
      createdInteraction,
      expect.any(Object),
    );
